                   ...
                   int domain, int *type, void *address, int initial )
{
     ...
     else  /* Restore the saved information. */
     {
          ...
          /* Restore the numeric IPv4 address string. */

          errno = 0;
          if ( inet_ntop( AF_INET, &( server.sin_addr.s_addr ), ip_str,
                          16 ) == NULL )
          {
               save_errno = errno;
               printf( "\n\
Something went wrong when calling inet_ntop(3).\n" );
          ...
          }    /* if ( inet_ntop( AF_INET, &( server.sin_addr.s_addr ),
                                  ip_str, 16 ) == NULL ) */
          ...
          *type = sock_type;
...
#ifndef USE_DEFAULT_TARGET_AF_INET
          ...
          if ( setup_address == 1 )
          {
               ...
               do
               {
                    ...
                    {
                         printf( "\n\
What numeric IPv4 address would you like the server to use?\n\
Please use dot decimal notation.  Do not specify a port number.\n\n" );
                    }
                    else
                    {
                         printf( "\n\
What numeric IPv4 address would you like the client to connect to?\n\
Please use dot decimal notation.  Do not specify a port number.\n\n" );
                    }
                    errno = 0;
                    ret = read_stdin( buffer, 32, ">> ", 1 );
...
network address for a numeric IPv4 address.\n" );
                         }
                         if ( save_errno != 0 )
                         {
                              printf( "Error: %s.\n",
               ...
               /*
                    ...
                    Make a copy of the numeric IPv4 address string
...
That is not an acceptable port number.  Please try again.\n" );
     ...
     if ( sock_type != SOCK_DGRAM )
     {
          ...
          ibox.num = server.sin_addr.s_addr;
          for( count = 0; count < 3; count++ )
          {
               printf( "%02X ", ( unsigned char )ibox.bytes[ count ] );
          }
          printf( "%02X\n", ( unsigned char )ibox.bytes[ 3 ] );
          printf( "and server port: " );
          sbox.num = server.sin_port;
     ...
     if ( sock_type != SOCK_DGRAM )
     {
          ...
          if ( use_server == 1 )
          {
               ...
               if ( inet_ntop( AF_INET,
                               &( listen_addr.sin_addr.s_addr ), buffer,
                               32 ) == NULL )
               {
                    save_errno = errno;
                    printf( "\
Something went wrong while using inet_ntop(3).\n" );
               ...
               {
                    printf( "\
The server's listening socket address is listed as %s:", buffer );
                    if ( endian == 1 )
                    {
                         printf( "\
%u.\n", ( uint16_t )ntohs( listen_addr.sin_port ) );
                    }
                    else
                    {
                         printf( "\
%u.\n", ( uint16_t )listen_addr.sin_port );
               ...
               errno = 0;
               if ( inet_ntop( AF_INET,
                               &( server_addr.sin_addr.s_addr ), buffer,
                               32 ) == NULL )
               {
                    save_errno = errno;
                    printf( "\n\
Something went wrong while using inet_ntop(3).\n" );
               ...
               {
                    printf( "\
The server socket's address is listed as %s:", buffer );
                    if ( endian == 1 )
                    {
                         printf( "\
%u.\n", ( uint16_t )ntohs( server_addr.sin_port ) );
                    }
                    else
                    {
                         printf( "\
...
Something went wrong while using inet_ntop(3).\n" );
                    ...
                    {
                         printf( "\
The client socket's address is listed as %s:", buffer );
                         if ( endian == 1 )
                         {
                              printf( "\
%u.\n", ( uint16_t )ntohs( client_addr.sin_port ) );
                         }
                         else
                         {
                              printf( "\
